...
public class ContextualImageFlowPrimaryService : ContextualFlow, ITransient
{
    ...
    public Task<FileInfo> RunFlow(
        string imagePath,
        string primaryText,
        string aspectRatio,
        int width, int height)
        => Run(imagePath: imagePath, primaryText: primaryText, secondaryText: string.Empty, aspectRatio: aspectRatio, width: width, height: height);
        ...
        bool debug = false)
    {
        ...
        var placements = TextPlacementHelper.FindPlacements(resizedCropped, new List<TextPlacementRequest>
        {
            new TextPlacementRequest
            {
                ...
                MinFontSize = width * 0.051f,
                MaxFontSize = width * 0.06f,
            },
        }, focalPoint);
